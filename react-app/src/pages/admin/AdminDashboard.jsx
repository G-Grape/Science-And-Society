...
import { FileText, Users, CheckCircle, Clock, ArrowRight, TrendingUp, Mail, Check } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  async function fetchAll() {
    ...
    const [journalsRes, profilesRes, pendingRes, approvedRes, recentJRes, recentURes, reqRes] = await Promise.all([
      ...
      supabase.from('journals').select('id', { count: 'exact', head: true }).in('status', ['approved', 'Accepted']),
    ...
    <div className="space-y-6">
        ...
        <div className="card">
          <div className="card-header">
            <div className="section-card-header">
              ...
              <Link to="/admin/journals" className="btn btn-outline btn-sm">View All <ArrowRight size={14} /></Link>
            </div>
          </div>
        ...
        <div className="card">
          <div className="card-header">
            <div className="section-card-header">
              ...
              <Link to="/admin/users" className="btn btn-outline btn-sm">View All <ArrowRight size={14} /></Link>
            </div>
          </div>
          <div className="card-content space-y-4">
      ...
      <div className="card">
        <div className="card-header">
          <div className="card-title" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Mail size={18} className="text-primary" /> Paper Access Requests
          </div>
              ...
              <div>
                ...
                <div style={{ marginTop: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <a href={`mailto:${req.requester_email}?subject=Paper Access: ${encodeURIComponent(req.journal_title)}`} className="btn btn-outline btn-sm">
                    <Mail size={14} /> Email PDF to {req.requester_email}
                  </a>
                </div>
              </div>
      ...
      <div className="card">
        ...
        <div className="card-content actions-list">
          <Link to="/admin/journals"  className="btn btn-outline" style={{ justifyContent: 'flex-start' }}><FileText size={16} /> Manage Journals</Link>
          <Link to="/admin/users"     className="btn btn-outline" style={{ justifyContent: 'flex-start' }}><Users size={16} /> Manage Users</Link>
          <Link to="/admin/reviewers" className="btn btn-outline" style={{ justifyContent: 'flex-start' }}><CheckCircle size={16} /> Assign Reviewers</Link>
        </div>
      </div>
    </div>
  )
